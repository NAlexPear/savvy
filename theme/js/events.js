...
(function iife(window, document, $, _, Helpers, Click, Scroll, Links) {
    ...
    $('#cta > .pointer-after').css('left', Helpers.getArrowPosition($('button[name="stl"]')));
    ...
    $(window).on('resize', function windowResizeHandler() {
        var debouncedArrowAdjustment = _.debounce(function adjustArrow() {
            $('#cta > .pointer-after').css('left', Helpers.getArrowPosition(window.city));
        }, 250);
